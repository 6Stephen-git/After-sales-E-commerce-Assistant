"""
Agent1 rule_match_plan 合并逻辑测试：特殊品类 C 通道锁定与检索词补全。
"""
...
class TestCategoryLaneLock:
    def test_infer_phone_slug_from_chat(self):
        """聊天含手机关键词时应推断 phone slug。"""
        slug = infer_category_slug_from_text("买家说手机屏幕有划痕，要求退款")
        assert slug == "phone"

    def test_infer_fresh_slug_from_banana_spoilage(self):
    ...
    def test_text_inference_injects_phone_doc_without_api_slug(self):
        """无 API 标时，文本推断手机品类应强制加入品类规范。"""
        plan = merge_llm_rule_plan(
            raw_plan={
                "activated_lanes": ["A"],
                "target_doc_ids": [],
...
class TestLexiconSearchEnrichment:
    ...
    def test_prompt_includes_rule_terms_and_alias_hint(self):
        """prompt 候选节应展示 rule_terms 与聊天命中的口语映射。"""
