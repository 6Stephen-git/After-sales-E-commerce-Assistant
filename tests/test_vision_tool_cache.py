"""
V/T 层缓存单元测试：视觉 API、画像、判例、物流的命中与 bypass。
"""
...
from backend.cache.tool_cache import clear_tool_caches
...
def reset_cache_state() -> None:
    """
    每个用例前清理纠纷缓存与 V/T 缓存，并重置 Redis 客户端单例。
    """
    clear_all_cache()
    clear_tool_caches()
    reset_redis_client()
...
def _mock_redis_storage() -> tuple[MagicMock, dict[str, str]]:
    """
    构造内存 Redis mock，供 patch get_redis 使用。
    """
...
# ---------- V 层：同图同 guidance 命中，error 不写入 ----------
def test_vision_cache_should_hit_after_save() -> None:
    """
    写入 V 层后，相同 image_ref + model + guidance 应命中缓存。
    """
    ...
    mock_client, storage = _mock_redis_storage()
    with patch.dict(os.environ, {"ENABLE_REDIS_CACHE": "1"}, clear=False):
        with patch("backend.cache.layer_redis.get_redis", return_value=mock_client):
            save_vision(image_ref, model, guidance, payload)
            cached = get_cached_vision(image_ref, model, guidance)

    assert cached is not None
    assert cached["visual_description"] == "可见划痕"


def test_vision_cache_should_miss_when_guidance_changes() -> None:
    """
    guidance 变化时 V 层应 miss。
    """
    image_ref = "mock://product-image"
    model = "qwen3-vl-flash"
    payload = {"visual_description": "可见划痕"}

    mock_client, _storage = _mock_redis_storage()
    with patch.dict(os.environ, {"ENABLE_REDIS_CACHE": "1"}, clear=False):
        with patch("backend.cache.layer_redis.get_redis", return_value=mock_client):
            save_vision(image_ref, model, "诉求A", payload)
            cached = get_cached_vision(image_ref, model, "诉求B")
...
def test_analyze_image_should_use_cache_without_second_http_call() -> None:
            ...
            response.json.return_value = {
                "output": {
                    "choices": [
                        {
                            "message": {
                                "content": json_module.dumps(vision_payload, ensure_ascii=False),
                            }
                        }
                    ]
                }
    ...
    assert first["visual_description"] == "包装破损"
    assert http_call_count["count"] == 1
    assert second == first


# ---------- T 层：物流命中 ----------
def test_logistics_cache_should_hit_on_second_query() -> None:
    """
    同 order_id 二次查询物流应命中 T 层。
    """
    mock_client, _storage = _mock_redis_storage()
    with patch.dict(os.environ, {"ENABLE_REDIS_CACHE": "1"}, clear=False):
        with patch("backend.cache.layer_redis.get_redis", return_value=mock_client):
            first = query_logistics("ORDER-9001")
            second = query_logistics("ORDER-9001")

    assert isinstance(first, LogisticsInfo)
    assert second == first


# ---------- T 层：判例命中 ----------
def test_cases_cache_should_hit_for_same_desc() -> None:
    """
    同 dispute_desc + top_k 二次检索应命中 T 层。
    """
    mock_client, _storage = _mock_redis_storage()
    with patch.dict(os.environ, {"ENABLE_REDIS_CACHE": "1"}, clear=False):
        with patch("backend.cache.layer_redis.get_redis", return_value=mock_client):
            first = search_similar_cases("买家称手机划痕", top_k=3)
            second = search_similar_cases("买家称手机划痕", top_k=3)

    assert first == []
    assert second == []


def test_cases_cache_should_miss_when_desc_changes() -> None:
    """
    dispute_desc 变化时判例缓存应 miss（独立写入）。
    """
...
def test_profile_cache_should_hit_without_second_db_query() -> None:
    ...
    assert first.credit_level == "good"
...
def test_tool_cache_should_bypass_when_redis_disabled() -> None:
    ...
    with patch.dict(os.environ, {"ENABLE_REDIS_CACHE": "0"}, clear=False):
        ...
        logistics = query_logistics("ORDER-1")
        assert isinstance(logistics, LogisticsInfo)
