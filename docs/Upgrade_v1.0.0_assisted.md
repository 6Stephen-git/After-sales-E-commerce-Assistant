...
| 编号   | 状态      | 备注                |
| ---- | ------- | ----------------- |
| M1   | done    | 2026-05-07：已实现 `backend/tools/llm_client.py`，支持缺省降级与重试。 |
| M2-1 | done    | 2026-05-07：已接入 LLM 事实守门，支持模糊不判与补证优先。 |
| M2-2 | done    | 2026-05-07：已接入 LLM reasoning，保持原决策打分逻辑不变。 |
| M2-3 | done    | 2026-05-07：已接入风格感知话术生成，失败自动模板兜底。 |
| M2-4 | pending |                   |
| M3-1 | pending |                   |
| M3-2 | pending |                   |
| M4-1 | pending |                   |
...
| 2026-05-07 | M1：新增 LLM 共用客户端，封装 endpoint/key、重试、中文错误日志与降级返回。 |
| 2026-05-07 | M2-1/2/3：Agent1/2/3 完成 LLM 升级与降级兜底，补充回归测试并通过。 |
