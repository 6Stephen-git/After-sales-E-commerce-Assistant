...
| 2026-05-05 | v0.5.0-frontend   | 新增  | 阶段五前端辅助模式：Vue3 路由 + 左右分栏（聊天/策略侧栏）、Axios `/api` 封装、枚举集中映射、Vitest 单测、`/dispute` 与 `/settings`、生产构建与代理联调就绪 | frontend/src（api、router、composables、components、views）、frontend/vite.config.js、frontend/package.json、frontend/package-lock.json |
| 2026-05-23 | v0.2.5-redis-cache | 新增 | Redis 三层缓存：材料/Agent1 事实/整报告分层存储，指纹失效 + 降级策略；AssistedController 集成 C/B 短路，流式与非流式共用 | backend/cache/, backend/controllers/assisted_controller.py, tests/test_dispute_cache.py, tests/test_assisted_controller.py, .env.example |
| 2026-05-23 | v0.2.6-rule-llm-match | 修改 | 规则匹配改为 LLM 主路径：品类 slug 结构化推断 + 候选条文语义选型；移除关键词 slug 猜测主路径 | backend/tools/rule_matcher_llm.py, backend/tools/rule_lexicon.py, backend/tools/rule_matcher.py, backend/agents/agent1/ |
