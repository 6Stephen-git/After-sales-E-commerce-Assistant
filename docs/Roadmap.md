...
- [x] 左右分栏布局（聊天窗口 + AI 策略面板）
- [x] 「请求 AI 帮助」按钮：商家手动触发分析
- [x] 侧边栏展示分析报告、话术选项（点击使用填入聊天框）
- [x] 前端与后端 API 联调测试（Vite `/api` 代理 FastAPI；`/analyze` 与 `/merchants/.../config` 走通）
- [x] 核心里程碑：辅助模式前端完整可用
- [x] **合并到 dev，打 tag `v0.5.0-frontend`，更新 Changelog**
