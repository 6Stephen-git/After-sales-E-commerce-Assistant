...
if __name__ == "__main__":
    ...
    case1 = monitor(
        text="太差了！我要投诉！",
        context={"alert_threshold": 0.8, "chat_history": ["之前说好今天处理，你们一直拖"]},
    )
    assert case1.alert_triggered is True, f"期望触发预警，实际={case1.alert_triggered}"
    assert case1.sentiment == "negative", f"期望negative，实际={case1.sentiment}"

    # ---------- 内嵌测试 2：温和负面不触发预警 ----------
    case2 = monitor(
        text="感觉和图片有点不一样",
        context={"alert_threshold": 0.8, "chat_history": ["衣服有点问题，想确认怎么处理"]},
    )
    assert case2.alert_triggered is False, f"期望不触发预警，实际={case2.alert_triggered}"
    assert case2.sentiment == "negative", f"期望negative，实际={case2.sentiment}"

    # ---------- 内嵌测试 3：正面且历史负面，注释应体现“勉强” ----------
    case3 = monitor(
        text="好吧，算了",
        context={
            "chat_history": ["这次购物很失望，我准备投诉平台", "你们一直没给清楚答复"],
            "alert_threshold": 0.8,
        },
    )
    assert case3.sentiment == "positive", f"期望positive，实际={case3.sentiment}"
    assert case3.emotion_note and "勉强" in case3.emotion_note, f"期望emotion_note含'勉强'，实际={case3.emotion_note}"

    # ---------- 内嵌测试 4：空文本边界 ----------
    case4 = monitor(text="", context={})
    assert case4.sentiment == "neutral", f"期望neutral，实际={case4.sentiment}"
    assert case4.intensity == 0.0, f"期望强度0.0，实际={case4.intensity}"
    assert case4.alert_triggered is False, f"期望不触发预警，实际={case4.alert_triggered}"

    print(f"{AGENT4_LOG_PREFIX} 内嵌测试全部通过")
